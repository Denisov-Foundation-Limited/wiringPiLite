...
static bool adsAnalogRead (struct wiringPiNodeStruct *node, int pin, int *value)
{
...
//	Start a single conversion
  ...
  if (wiringPiI2CWriteReg16 (node->fd, 1, config) < 0) {
    return false;
  }
...
static bool adsDigitalWrite (struct wiringPiNodeStruct *node, int pin, int data)
{
  int chan = pin - node->pinBase ;
  chan &= 3 ;
...
static bool adsAnalogWrite (struct wiringPiNodeStruct *node, int pin, int data)
{
  int chan = pin - node->pinBase ;
  int reg ;
  int16_t ndata ;
  ...
  if (wiringPiI2CWriteReg16 (node->fd, reg, data) < 0) {
    return false;
  }
  return true;
}
...
bool ads1115Setup (unsigned i2cBusId, int i2cAddr, const int pinBase)
{
  struct wiringPiNodeStruct *node ;
  int fd ;

  if (!wiringPiI2CSetup (i2cBusId, i2cAddr, &fd))
    return false ;
  ...
  node->analogRead   = adsAnalogRead ;
  node->analogWrite  = adsAnalogWrite ;
  node->digitalWrite = adsDigitalWrite ;
